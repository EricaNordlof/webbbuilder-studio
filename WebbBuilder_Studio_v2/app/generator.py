...
- Skapa inte låtsas-.jpg/.png-filer som text. Använd SVG eller fungerande externa HTTPS-bilder.
- För externa stockbilder: använd direkta bild-URL:er som returnerar image/*, inte söksidor eller redirect-tjänster.
- Använd inte source.unsplash.com eller vanliga unsplash.com-sidURL:er som <img src>.
- Om användaren begär ett bestämt antal bilder ska alla faktiskt implementeras.
- Om hero-bild begärs ska den finnas tydligt i hero-sektionen.
...
async def repair_project(
    ...
    """Repair only concrete quality failures found by deterministic QA."""
    ...
    prompt = f"""
REPARERA ETT BEFINTLIGT WEBBPROJEKT EFTER AUTOMATISK KVALITETSKONTROLL.
...
Senaste användarinstruktion:
{latest_instruction}

KVALITETSKONTROLLEN HITTADE DESSA KONKRETA FEL:
{issue_text}

VIKTIGT:
- Reparera samtliga blockerande fel ovan.
- Returnera hela projektets aktuella filuppsättning efter reparationen.
- Skriv kompletta filer, inte diffar.
- Behåll all fungerande funktionalitet och design som inte måste ändras.
- Påstå aldrig att en extern bild-URL fungerar bara för att den ser rimlig ut.
- När externa stockbilder används: använd direkta HTTPS-bildadresser som faktiskt
  returnerar en bildfil, inte en söksida, redirect-tjänst eller vanlig HTML-sida.
- Om ett krav anger ett visst antal bilder ska alla finnas som faktiska bildreferenser.
- Om en hero-bild efterfrågas ska den vara tydligt implementerad i hero-sektionen.
- Kontrollera lokala src/href-referenser så att de pekar på filer som finns i leveransen.
